...
class TaskManager:
    ...
    _loop: asyncio.AbstractEventLoop = field(
        init=False, default_factory=asyncio.new_event_loop
    )
    ...
    def _run_in_workthread(
        self, coroutine_func, append: bool = True, *args, **kwargs
    ) -> asyncio.Future:
        """
        ...
        cf_future = asyncio.run_coroutine_threadsafe(
            coroutine_func(*args, **kwargs), self._loop
        )
    ...
    def start(self) -> None:
        ...
        self._workthread = threading.Thread(
            target=self._start_execution, name=f"{self.name}_workthread", daemon=True
        )
    ...
    def stop_after(self, delay: int | float, /):
        ...
        return self.run_after(
            delay, self.stop, task_name=f"stop_{self.name}_after_{delay}seconds"
        )
    ...
    def stop_at(self, time: str, /):
        ...
        return self.run_at(
            time,
            self.stop,
            task_name=f"stop_{self.name}_at_{underscore_datetime(time)}",
        )
    ...
    def stop_on(self, datetime: str, /):
        ...
        return self.run_on(
            datetime,
            self.stop,
            task_name=f"stop_{self.name}_on_{underscore_datetime(datetime)}",
        )
